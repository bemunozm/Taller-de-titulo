...
export class AuthEmailService {
    ...
    async sendAccountCreatedByAdminEmail(user: { email: string; name: string }, token: string): Promise<void> {
        try {
            console.log('Intentando enviar email de cuenta creada por admin a:', user.email);

            const frontendUrl = this.configService.get('FRONTEND_URL') || 'http://localhost:3000';
            const emailHtml = this.generateAccountCreatedByAdminEmailTemplate(user.name, token, frontendUrl);

            const result = await this.mailerService.sendMail({
                to: user.email,
                subject: '🎉 Tu cuenta ha sido creada - Conserje Digital',
                html: emailHtml
            });

            console.log('Email de cuenta creada por admin enviado a:', user.email);
            console.log('Resultado:', result);
        } catch (error) {
            console.error('Error al enviar email de cuenta creada:', error);
            throw error;
        }
    }
    ...
    /**
     * Genera el template HTML para el email de cuenta creada por administrador
     */
    private generateAccountCreatedByAdminEmailTemplate(name: string, token: string, frontendUrl: string): string {
        const resetUrl = `${frontendUrl}/auth/new-password?token=${token}`;
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Bienvenido a Conserje Digital</title>
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                body { 
                    font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
        ...
        <body>
            <div class="container">
                <div class="header">
                    <div class="logo-space">LOGO</div>
                    <h1>¡Bienvenido a Conserje Digital!</h1>
                    <p>Tu cuenta ha sido creada</p>
                </div>
                <div class="content">
                    <div class="greeting">Hola ${name}</div>
                    <div class="message">
                        Un administrador ha creado una cuenta para ti en <strong>Conserje Digital</strong>. 
                        Para comenzar a usar la plataforma, necesitas establecer tu contraseña personal.
                    </div>
                    
                    <div class="highlight-box">
                        <div class="highlight-label">Tu código de verificación</div>
                        <div class="highlight-code">${token}</div>
                        <p style="color: #6b7280; font-size: 13px; margin-top: 12px;">
                            El botón de abajo completará automáticamente este código
                        </p>
                    </div>
                    ...
                    <div class="features">
                        <h3>¿Qué puedes hacer en Conserje Digital?</h3>
                        <ul class="feature-list">
                            <li>Gestionar tu perfil y configuración</li>
                            <li>Acceder a las funcionalidades de tu rol</li>
                            <li>Colaborar con tu equipo</li>
                            <li>Recibir notificaciones importantes</li>
                        </ul>
                    </div>
                ...
                <div class="footer">
                    ...
                    <p style="margin-top: 16px;">© 2024 Conserje Digital. Todos los derechos reservados.</p>
                </div>
            </div>
        </body>
        </html>
